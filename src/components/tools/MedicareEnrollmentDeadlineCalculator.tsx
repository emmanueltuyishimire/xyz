...
export const MedicareEnrollmentDeadlineCalculator: React.FC = () => {
  ...
  // Birth years: cover people aged 55–85 (who may be pre-planning or past age 65)
  const years65 = [];
  const currentYear = new Date().getFullYear();
  for (let y = currentYear - 55; y >= currentYear - 85; y--) {
    years65.push(y);
  }
          ...
          <div className="date-select-grid">
            ...
            <div className="date-select-col">
              ...
              >
                ...
                {years65.map(y => (
                  <option key={y} value={y}>{y} — 65th birthday: {y + 65}</option>
                ))}
              </select>
            </div>
          </div>
